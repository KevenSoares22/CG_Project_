...
const App = () => {
  ...
  useEffect(() => {
    ...
    // Flag para indicar se o primeiro coco já iniciou o deslize sobre a ilha
    ...
    // Define o contato entre coco e solo
    ...
    const animate = () => {
      ...
      // Quando o coco atingir aproximadamente a superfície da ilha (y < 8),
      // inicia o modo de deslize.
      ...
      if (coconutSliding && coconutBody1) {
        // Definimos uma velocidade horizontal desejada (diagonal em X e Z)
        const speed = 5;
        const vx = speed / Math.sqrt(2);
        const vz = speed / Math.sqrt(2);
        // Pega a posição atual horizontal do coco
        const x = coconutBody1.position.x;
        const z = coconutBody1.position.z;
        const r2 = x * x + z * z;
        // Calcula a altura ideal da ilha na posição (x,z)
        const islandY = groundHeight + 0.3 * Math.sqrt(Math.max(0, islandRadius * islandRadius - r2));
        // Calcula as derivadas parciais da função y(x,z) = groundHeight + 0.3*sqrt(R² - (x²+z²))
        const denom = Math.sqrt(Math.max(0.0001, islandRadius * islandRadius - r2));
        const dYdx = -0.3 * x / denom;
        const dYdz = -0.3 * z / denom;
        // Para que o movimento seja tangente à superfície, o componente vertical é:
        const vy = dYdx * vx + dYdz * vz;
        // Atualiza a velocidade do coco para seguir a curva da ilha
        coconutBody1.velocity.set(vx, vy, vz);
        // Garante que o coco fique na altura correta da superfície
        coconutBody1.position.y = islandY;
        // Se o coco chegar à borda da ilha (considerando o raio do coco), interrompe o deslize
    ...
    // Inicia a animação do sol após 1 segundo
    setTimeout(() => {
